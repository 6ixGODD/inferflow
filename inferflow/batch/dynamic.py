...
class DynamicBatchStrategy(BatchStrategy[P, R]):
    ...
    async def submit(self, item: P) -> R:
        ...
        if self._queue.full() and not self.block_on_full:
            ...
            raise QueueFullError(
                f"Queue is full ({self.queue_size} items). Rejected {self.metrics.rejected_requests} requests so far."
            )
    ...
    async def stop(self) -> None:
        ...
        logger.info(
            ...
            f"Avg batch size: {self.metrics.avg_batch_size:.1f}, "
    ...
    async def _worker(self) -> None:
        ...
        while self._running:
            ...
            try:
                # Wait for first item (with timeout to check _running flag)
                try:
                    first_item = await aio.wait_for(self._queue.get(), timeout=1.0)
                    batch.append(first_item)
                except TimeoutError:
                    continue  # No items, check _running and retry
    ...
    async def _process_batch(self, batch: list[tuple[P, aio.Future[R]]]) -> None:
        ...
        Args:
            batch: List of (input, future) tuples.
        """
        ...
        try:
            # Check if runtime supports batch inference
            if hasattr(self.runtime, "infer_batch"):
                results = await self.runtime.infer_batch(items)  # type: ignore
            else:
                # Fallback: sequential inference
                results = [await self.runtime.infer(item) for item in items]  # type: ignore
            ...
            logger.debug(
                ...
                f"throughput={batch_size / processing_time:.1f} req/s"
            )
